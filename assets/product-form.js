...
setTimeout(function (){
     if(document.querySelector('.on-off-btn.clickMEplz'))
     {
       runUpsell(); 
     }
      ...
      if (this.bundleDeals) {
        ...
        for (let i=0; i < this.bundleDeals.formVariants.length; i++) {
          const current = this.bundleDeals.formVariants[i];
          variantsHTML += `<input type="hidden" name="items[${i}][quantity]" value="${current.quantity}"><input type="hidden" name="items[${i}][id]" value="${current.id}">`;
        }
        this.variantInputs.innerHTML = variantsHTML;
        ...
        for (let i=0; i < this.quantityBreaks.formVariants.length; i++) {
          const current = this.quantityBreaks.formVariants[i];
          variantsHTML += `<input type="hidden" name="items[${i}][quantity]" value="${current.quantity}"><input type="hidden" name="items[${i}][id]" value="${current.id}">`;
        }
        this.variantInputs.innerHTML = variantsHTML;
      }
